 ...
 * ============================================================
 * File:            admin.js
 * Description:     Admin dashboard logic for PIN, services, and event controls.
 *
  ...
  if (pin) {
    ...
    if (pin.LastUpdated) {
      lastUpdated.textContent =
        "Last updated: " + formatChicagoTime(pin.LastUpdated);
    }
  }
  ...
  if (adminPin) {
    ...
    if (adminPin.LastUpdated) {
      adminLastUpdated.textContent =
        "Last updated: " + formatChicagoTime(adminPin.LastUpdated);
    }
  }
}
...
document
  .getElementById("btnTogglePinVisibility")
...
document
  .getElementById("btnToggleAdminPinVisibility")
  .addEventListener("click", () => {
  ...
  try {
    ...
    if (result.success) {
      Swal.fire({
        icon: "success",
        title: "Saved",
        text: "General PIN updated.",
        timer: 1500,
        showConfirmButton: false,
      });
      document.getElementById("pinLastUpdated").textContent =
        "Last updated: " + nowChicago();
    } else {
...
// Save admin PIN
document
  .getElementById("btnSaveAdminPin")
    ...
    try {
      ...
      if (result.success) {
        Swal.fire({
          icon: "success",
          title: "Saved",
          text: "Admin PIN updated.",
          timer: 1500,
          showConfirmButton: false,
        });
        document.getElementById("adminPinLastUpdated").textContent =
          "Last updated: " + nowChicago();
      } else {
        throw new Error(result.error || "Update failed");
      }
...
function renderServicesSection() {
  ...
  if (services.length === 0) {
    container.innerHTML =
      '<div class="text-center p-4 text-muted">No services found.</div>';
    return;
  }

  // Group: categories first, then their children underneath
  const categories = services.filter(
    (s) => s.ServiceType === "category" && !s.ParentServiceID,
  );
  const operationals = services.filter(
    (s) => s.ServiceType === "operational" && s.ParentServiceID,
  );
  const childMap = {};
  ...
  operationals
    .filter((op) => !categoryIDs.has(op.ParentServiceID))
    .forEach((op) => {
...
function renderServiceRow(svc, isChild) {
  ...
  const childIcon = isChild
    ? '<i class="bi bi-arrow-return-right text-muted me-1" style="font-size:0.75rem;"></i>'
    : "";
  ...
  return `
    <div class="${rowClass}" data-service-id="${svc.ServiceID}" data-event-service-id="${svc.EventServiceID || ""}" data-icon-tag="${(svc.IconTag || "").replace(/"/g, "&quot;")}">
        ...
        <div class="service-fields">
            ...
            `
                : !isChild
                  ? '<div class="text-muted small fst-italic">Category only — no event settings</div>'
...
async function handleToggleService(e) {
  const row = e.target.closest(".service-row");
  const eventServiceID = row.dataset.eventServiceId;
  const svc = adminData.services.find(
    (s) => s.EventServiceID === eventServiceID,
  );
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const parentServiceID =
      document.getElementById("newParentServiceID").value || null;
    const sortOrder =
      parseInt(document.getElementById("newSortOrder").value) || 0;
    ...
    if (!iconTag) {
      Swal.fire({
        icon: "error",
        title: "No Icon",
        text: "Please select an icon for the service.",
      });
      return;
    }
    ...
    if (serviceType === "operational" && !parentServiceID) {
      Swal.fire({
        ...
        text: "Operational services must have a parent category.",
      });
      return;
    }
...
function renderFastTrackSection() {
  ...
  document
    .getElementById("btnSaveFastTrack")
    .addEventListener("click", async () => {
      ...
      try {
        ...
        if (result.success) {
          Swal.fire({
            icon: "success",
            title: "Saved",
            text: `Fast track limit set to ${val}.`,
            timer: 1500,
            showConfirmButton: false,
          });
          await loadAdminData();
...
if (btnRefreshAll) {
  btnRefreshAll.addEventListener("click", () =>
    spinRefreshBtn(btnRefreshAll, loadAdminData()),
  );
}
