...
const serviceOutcomesTableBody = document.getElementById('serviceOutcomesTableBody');
const serviceOutcomesChartHint = document.getElementById('serviceOutcomesChartHint');
const serviceWaitTimesChartHint = document.getElementById('serviceWaitTimesChartHint');
const registrationChoiceCards = document.getElementById('registrationChoiceCards');
const registrationChoicesHint = document.getElementById('registrationChoicesHint');
...
const metricElements = {
    ...
    chiropractorVolunteersServed: document.getElementById('metricChiroVolunteers'),
    volunteerBadgesPrinted: document.getElementById('metricVolunteerBadgesPrinted'),
};
...
function updateReportButtonState(isBusy = false) {
    ...
    const hasReportData = Boolean(latestDashboardSnapshot && latestDashboardSnapshot.selectedEvent);
...
function buildReportPdfDoc(reportText, eventName) {
    ...
    lines.forEach((line) => {
        const wrapped = line.trim() === '' ? [' '] : doc.splitTextToSize(line, contentWidth);
...
function buildReportFileBase(eventID) {
    const cleanedEventID = String(eventID || 'event').replace(/[^a-zA-Z0-9_-]/g, '_');
...
function buildDashboardReport(snapshot) {
    ...
    const completionInterpretation = overallCompletionRate >= 85
    ...
    const registrationTotal = registrationChoices.reduce((sum, row) => sum + safeNumber(row.chosenCount), 0);
    const hasRegistrationPast = registrationChoices.some((row) => row.pastChosenCount !== null && row.pastChosenCount !== undefined);
    const registrationPastTotal = hasRegistrationPast
        ? registrationChoices.reduce((sum, row) => sum + safeNumber(row.pastChosenCount), 0)
        : 0;
    ...
    registrationChoices.forEach((row) => {
        ...
        const pastCount = row.pastChosenCount === null || row.pastChosenCount === undefined
            ? null
            : safeNumber(row.pastChosenCount);
        const delta = pastCount === null ? null : currentCount - pastCount;

        if (!topRegistrationChoice || currentCount > safeNumber(topRegistrationChoice.count)) {
        ...
        if (delta !== null) {
            if (!largestRegistrationGain || delta > safeNumber(largestRegistrationGain.delta)) {
            ...
            if (!largestRegistrationDrop || delta < safeNumber(largestRegistrationDrop.delta)) {
    ...
    const waitWithSelected = waitComparisons.filter((row) => row.selectedAvgMinutes !== null && row.selectedAvgMinutes !== undefined);
    const waitComparable = waitComparisons.filter((row) => (
        row.selectedAvgMinutes !== null && row.selectedAvgMinutes !== undefined &&
        row.pastAvgMinutes !== null && row.pastAvgMinutes !== undefined
    ));

    const selectedAvgOverall = waitWithSelected.length > 0
        ? waitWithSelected.reduce((sum, row) => sum + safeNumber(row.selectedAvgMinutes), 0) / waitWithSelected.length
        : 0;

    const pastAvgOverall = waitComparable.length > 0
        ? waitComparable.reduce((sum, row) => sum + safeNumber(row.pastAvgMinutes), 0) / waitComparable.length
        : 0;

    const overallWaitDelta = waitComparable.length > 0 ? (selectedAvgOverall - pastAvgOverall) : null;
    const fasterOrEqualCount = waitComparable.filter((row) => safeNumber(row.deltaMinutes) <= 0).length;
    const slowerCount = waitComparable.filter((row) => safeNumber(row.deltaMinutes) > 0).length;
    ...
    lines.push('- Abandoned visits: ' + formatNumber(abandoned) + ' (' + abandonmentRate.toFixed(1) + '% of visits)');
    lines.push('- Needs translator: ' + formatNumber(metrics.translatorNeeded));
    lines.push('- Volunteer badges printed (unique): ' + formatNumber(metrics.volunteerBadgesPrinted));
    lines.push('- Food truck clients / volunteers: ' + formatNumber(metrics.clientsServed) + ' / ' + formatNumber(metrics.volunteersServed));
    lines.push('- Chiro clients / volunteers: ' + formatNumber(metrics.chiropractorClientsServed) + ' / ' + formatNumber(metrics.chiropractorVolunteersServed));
    lines.push('');
    ...
    lines.push('- Overall completion rate: ' + overallCompletionRate.toFixed(1) + '%');
    if (largestGapRow && largestGapRow.gap > 0) {
        lines.push('- Largest unfinished gap: ' + largestGapRow.name + ' (' + formatNumber(largestGapRow.gap) + ' not completed)');
    }
    ...
    if (topRegistrationChoice) {
        lines.push('- Top selected service: ' + topRegistrationChoice.name + ' (' + formatNumber(topRegistrationChoice.count) + ')');
    } else {
        lines.push('- No registration choice data is available for this event.');
    }

    if (hasRegistrationPast) {
        lines.push('- Total selections vs past event: ' + formatNumber(registrationTotal) + ' vs ' + formatNumber(registrationPastTotal) + ' (' + formatSigned(registrationDelta, 0) + ')');
        if (largestRegistrationGain) {
            lines.push('- Largest increase: ' + largestRegistrationGain.name + ' (' + formatSigned(largestRegistrationGain.delta, 0) + ')');
        }
        if (largestRegistrationDrop) {
            lines.push('- Largest decrease: ' + largestRegistrationDrop.name + ' (' + formatSigned(largestRegistrationDrop.delta, 0) + ')');
        }
    } else {
        lines.push('- No past-event registration baseline is available for comparison.');
    }
    lines.push('');

    lines.push('4) Avg Service Time vs Past Event');
    lines.push('- Current event average across services: ' + selectedAvgOverall.toFixed(1) + ' min');
    if (overallWaitDelta !== null) {
        lines.push('- Past event average across comparable services: ' + pastAvgOverall.toFixed(1) + ' min');
        lines.push('- Net change vs past: ' + formatSigned(overallWaitDelta, 1) + ' min');
        lines.push('- Services faster/equal: ' + formatNumber(fasterOrEqualCount) + '; services slower: ' + formatNumber(slowerCount));
        if (largestSlowdown && largestSlowdown.delta > 0) {
            lines.push('- Biggest slowdown: ' + largestSlowdown.name + ' (' + formatSigned(largestSlowdown.delta, 1) + ' min)');
        }
        if (bestImprovement && bestImprovement.delta < 0) {
            lines.push('- Biggest improvement: ' + bestImprovement.name + ' (' + formatSigned(bestImprovement.delta, 1) + ' min)');
        }
    } else {
        lines.push('- Past-event service time data is not available for comparison.');
    }
    lines.push('');

    lines.push('5) What This Means');
    if (overallCompletionRate < 70) {
        lines.push('- Priority: reduce unfinished services first; completion is below target.');
    } else if (overallCompletionRate < 85) {
        lines.push('- Priority: improve completion consistency in high-gap services.');
    } else {
        lines.push('- Throughput is healthy; maintain staffing patterns that support completion.');
    }

    if (overallWaitDelta !== null && overallWaitDelta > 0.5) {
        lines.push('- Wait times are trending slower than the past event; review flow in the slowdown services.');
    } else if (overallWaitDelta !== null && overallWaitDelta < -0.5) {
        lines.push('- Wait times improved versus past event; current process changes appear effective.');
    }

    if (abandonmentRate > 15) {
        lines.push('- Abandonment is elevated; consider front-desk triage or expectation-setting at check-in.');
    }
...
async function showGeneratedReport(reportText, eventID) {
    ...
    const eventName = latestDashboardSnapshot && latestDashboardSnapshot.selectedEvent
    ...
    const result = await Swal.fire({
        ...
        html: '<div class="text-start">'
            ...
            + '<iframe title="Report PDF Preview" src="' + previewUrl + '" style="width:100%; height:65vh; border:1px solid #e2e8f0; border-radius:8px; background:#fff;"></iframe>'
            + '</div>',
...
function renderServiceOutcomes(rows) {
    ...
    serviceOutcomesTableBody.innerHTML = rows.map((row) => {
...
function renderRegistrationChoiceCards(rows) {
    ...
    if (!rows || rows.length === 0) {
        registrationChoiceCards.innerHTML = '<div class="text-muted small">No registration choice data.</div>';
        return;
    }

    const hasPastComparison = rows.some((row) => row.pastChosenCount !== null && row.pastChosenCount !== undefined);

    registrationChoiceCards.innerHTML = rows.map((row) => {
...
function renderRegistrationChoices(rows) {
    ...
    const hasPastComparison = safeRows.some((row) => row.pastChosenCount !== null && row.pastChosenCount !== undefined);
    ...
    if (labels.length === 0) {
        registrationChoicesHint.textContent = 'No registration step-5 selections for this event yet.';
        if (registrationChoicesChart) {
            registrationChoicesChart.destroy();
            registrationChoicesChart = null;
        }
...
function renderServiceOutcomesChart(data) {
    ...
    if (labels.length === 0) {
        serviceOutcomesChartHint.textContent = 'No service outcome records for this event yet.';
        if (serviceOutcomesChart) {
            serviceOutcomesChart.destroy();
            serviceOutcomesChart = null;
        }
...
function renderServiceWaitTimesChart(data) {
    ...
    const labels = data.labels || [];
    const selectedSeries = (data.selectedAvgMinutes || []).map((value) => Number(value || 0));
    const pastSeries = (data.pastAvgMinutes || []).map((value) => Number(value || 0));
    const pastEventID = data.pastEventID || null;

    if (labels.length === 0) {
        serviceWaitTimesChartHint.textContent = 'No service time records found for this event.';
        if (serviceWaitTimesChart) {
            serviceWaitTimesChart.destroy();
            serviceWaitTimesChart = null;
        }
    ...
    const hasPastComparison = Boolean(
        pastEventID && pastSeries.some((value) => Number.isFinite(value) && value > 0)
    );

    if (!hasPastComparison) {
        serviceWaitTimesChartHint.textContent = 'Showing selected-event average service times.';
    } else {
        serviceWaitTimesChartHint.textContent = 'Selected bars are green when faster than past avg and red when slower.';
    }
                    ...
                    callbacks: {
                        label(context) {
                            ...
                            if (hasPastComparison && Number.isFinite(past)) {
                                ...
                                const deltaPrefix = delta > 0 ? '+' : '';
                                return 'Selected: ' + selected.toFixed(1) + ' min (Past: ' + past.toFixed(1) + ', ' + deltaPrefix + delta.toFixed(1) + ', ' + relation + ')';
                            }
...
async function loadDashboard(eventID = '') {
    ...
    try {
        ...
        renderServiceWaitTimesChart((data.charts || {}).serviceWaitTimes || {
            labels: [],
            selectedAvgMinutes: [],
            pastAvgMinutes: [],
            pastEventID: null,
        });
        renderServiceOutcomesChart((data.charts || {}).serviceOutcomes || {
            labels: [],
            assigned: [],
            completed: [],
        });
...
function bindEvents() {
    ...
    if (generateReportBtn) {
        generateReportBtn.addEventListener('click', async () => {
            ...
            await showGeneratedReport(reportText, latestDashboardSnapshot.selectedEvent.EventID);
        });
    }
}
