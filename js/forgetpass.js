 ...
 * ============================================================
 * File:            forgetpass.js
 * Description:     Forgot password verification and reset workflow.
 *
...
function createDobMask(lang) {
  ...
  const currentTypedDate =
    dobMask && dobMask.typedValue instanceof Date && !isNaN(dobMask.typedValue)
...
function updatePasswordFieldMarkers() {
  ...
  if (!confirm) {
    ...
    const matches =
      password === confirm && VALIDATION_PATTERNS.password.test(password);
    ...
    if (!data.success) {
      throw new Error(
        getText(
          ...
          "Email and Date of Birth combination are incorrect",
        ),
      );
    }
