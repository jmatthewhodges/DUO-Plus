 ...
 * ============================================================
 * File:            register.js
 * Description:     Multi-step registration validation and submission flow.
 *
...
function createDobMask(lang) {
  ...
  const previousTypedDate =
    dobMask &&
    dobMask.masked?.isComplete &&
    dobMask.typedValue instanceof Date &&
    !isNaN(dobMask.typedValue)
...
function applyWaiverLanguage(lang) {
  ...
  waiverContent.innerHTML =
    lang === "es" ? WAIVER_CONTENT_ES_HTML : waiverContentDefaultHtml;
}
...
async function loadServiceCategories() {
  ...
  try {
    ...
    openCategories.forEach((cat) => {
      ...
      const displayName = translateServiceName(
        cat.ServiceName,
        cat.ServiceID,
        currentLang,
      );
    ...
    const serviceLegend =
      t.serviceSelectionLegend || "Select the services you need";
    grid.innerHTML = `
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("selLanguageSwitch")
...
document
  .getElementById("clientRegisterFormStep1")
...
document
  .getElementById("btnRegisterNext1")
  .addEventListener("click", stepOneSubmit);

document
  .getElementById("btnRegisterBack1")
...
function stepTwoSubmit() {
  ...
  if (
    ...
    !VALIDATION_PATTERNS.phoneFormatted.test(phone.value)
  ) {
...
document
  .getElementById("clientRegisterFormStep2")
  .addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
...
document
  .getElementById("btnRegisterNext2")
  .addEventListener("click", stepTwoSubmit);

document
  .getElementById("btnRegisterBack2")
  .addEventListener("click", function () {
...
// Step 3 validation
document
  .getElementById("btnRegisterNext3")
  .addEventListener("click", function () {
...
document
  .getElementById("btnRegisterBack3")
  .addEventListener("click", function () {
    const stepTwo = document.getElementById("divStepTwo");
    const stepThree = document.getElementById("divStepThree");
...
document
  .getElementById("clientRegisterFormStep3")
  .addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
...
const noEmergencyContactCheckbox =
  document.getElementById("noEmergencyContact");
...
// Step 4 validation
document
  .getElementById("btnRegisterNext4")
  .addEventListener("click", function () {
    const noEmergencyContact =
      document.getElementById("noEmergencyContact").checked;
...
document
  .getElementById("btnRegisterBack4")
  .addEventListener("click", function () {
    const stepThree = document.getElementById("divStepThree");
    const stepFour = document.getElementById("divStepFour");
...
document
  .getElementById("clientRegisterFormStep4")
  .addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
...
document
  .getElementById("btnRegisterNext5")
  .addEventListener("click", function () {
    const services = document.querySelectorAll(
      'input[name="clientServices"]:checked',
    );
...
document
  .getElementById("btnRegisterBack5")
  .addEventListener("click", function () {
    const stepFour = document.getElementById("divStepFour");
    const stepFive = document.getElementById("divStepFive");
  ...
  .addEventListener("blur", async function () {
    ...
    try {
      const res = await fetch(
        `../api/CheckEmail.php?email=${encodeURIComponent(val)}`,
      );
      ...
      if (data.exists) {
        ...
        feedbackEl.innerHTML =
          'An account with this email already exists. <a href="../index.html" class="text-danger fw-semibold">Sign in instead →</a>';
...
document
  .getElementById("clientRegisterPass")
...
document
  .getElementById("clientMiddleInitial")
  .addEventListener("blur", function () {
    if (this.value.trim()) {
      this.classList.add("is-valid");
    } else {
...
document.getElementById("clientDOB").addEventListener("input", function () {
  clearInvalid(this);
});
...
document
  .getElementById("clientAddress1")
  .addEventListener("input", function () {
    this.value = this.value.replace(INPUT_FILTERS.address, "");
  });

document
  .getElementById("clientAddress2")
  .addEventListener("input", function () {
    this.value = this.value.replace(INPUT_FILTERS.address, "");
  });
...
document
  .getElementById("emergencyContactFirstName")
  .addEventListener("input", function (e) {
    e.target.value = e.target.value.replace(INPUT_FILTERS.name, "");
  });

document
  .getElementById("emergencyContactLastName")
  .addEventListener("input", function (e) {
    e.target.value = e.target.value.replace(INPUT_FILTERS.name, "");
  });
...
document
  .getElementById("btnWaiverSubmit")
  .addEventListener("click", function () {
    const btn = this;
    ...
    if (clientIDFromUrl) {
      ...
      formData = {
        ...
        sex:
          document.querySelector('input[name="clientSex"]:checked')?.value ||
          "",
        ...
        noEmergencyContact:
          document.getElementById("noEmergencyContact").checked,
        emergencyFirstName: document.getElementById("emergencyContactFirstName")
          .value,
        emergencyLastName: document.getElementById("emergencyContactLastName")
          .value,
...
document
  .getElementById("skipStep1")
  .addEventListener("click", () => showStepOnly(1));
document
  .getElementById("skipStep2")
  .addEventListener("click", () => showStepOnly(2));
document
  .getElementById("skipStep3")
  .addEventListener("click", () => showStepOnly(3));
document
  .getElementById("skipStep4")
  .addEventListener("click", () => showStepOnly(4));
document
  .getElementById("skipStep5")
  .addEventListener("click", () => showStepOnly(5));
