 ...
 * Description:     Registration dashboard queue, check-in, and badge tools.
 *
    ...
    serviceCategories.forEach((cat) => {
      ...
      if (cat.children && cat.children.length > 0) {
        cat.children.forEach((child) => {
          const containerId =
            "service-" +
            child.ServiceID.replace(/([A-Z])/g, "-$1").toLowerCase();
          // Strip parent name prefix from child (e.g. "Medical Exam" → "Exam")
          let shortName = child.ServiceName;
          if (
            shortName.toLowerCase().startsWith(cat.ServiceName.toLowerCase())
          ) {
            shortName = shortName
          ...
          serviceMapping[child.ServiceID] = {
            ...
            displayName:
              cat.ServiceName + " - " + (shortName || child.ServiceName),
          };
        });
      } else {
  ...
  Object.entries(serviceMapping).forEach(([serviceID, mapping]) => {
    ...
    for (const cat of serviceCategories) {
      if (cat.children && cat.children.length > 0) {
        ...
        if (child) {
          iconTag = child.IconTag || cat.IconTag || "";
          break;
        }
...
function buildQrIconSlots(container, selectedServiceIDs, iconLookup) {
  ...
  selectedServiceIDs.forEach((svcID) => {
    ...
    if (directCat) {
      selectedCategoryMap[svcID] = svcID;
      return;
    }
  ...
  serviceCategories.forEach((cat) => {
    ...
    if (selectedChildID) {
      ...
      wrapper.innerHTML = renderIcon(
        iconLookup[selectedChildID] || iconLookup[cat.ServiceID] || "bi-circle",
      );
    } else {
...
const closeChangePasswordModalBtn = document.getElementById(
...
const clearUserPasswordSearchBtn = document.getElementById(
  "clearUserPasswordSearchBtn",
);
const passwordUserTableBody = document.getElementById("passwordUserTableBody");
const selectedPasswordUserName = document.getElementById(
  "selectedPasswordUserName",
);
const selectedPasswordUserDob = document.getElementById(
  "selectedPasswordUserDob",
);
const selectedPasswordUserEmail = document.getElementById(
  "selectedPasswordUserEmail",
);
const selectedPasswordUserPhone = document.getElementById(
  "selectedPasswordUserPhone",
);
const newUserPassword = document.getElementById("newUserPassword");
const toggleNewUserPasswordBtn = document.getElementById(
  "toggleNewUserPasswordBtn",
);
...
const toggleConfirmUserPasswordBtn = document.getElementById(
  "toggleConfirmUserPasswordBtn",
);
const confirmUserPasswordIcon = document.getElementById(
  "confirmUserPasswordIcon",
);
...
function normalizeServiceIds(serviceIds) {
  return [
    ...new Set(
      (serviceIds || []).map((id) => String(id).trim()).filter(Boolean),
    ),
  ].sort();
}
...
function validatePasswordFieldsLive(requireBoth = false) {
  ...
  const passwordInvalid =
    (requireBoth && !hasPassword) || invalidFormat || mismatch;

  const confirmInvalid = (requireBoth && !hasConfirm) || mismatch;
...
function updateServiceProgressBars(servicesData) {
  ...
  servicesData.forEach((s) => {
    closedLookup[s.serviceID] = !!s.isClosed;
  });
  ...
  serviceCategories.forEach((cat) => {
    if (cat.children && cat.children.length > 0) {
      ...
      serviceAvailability[cat.ServiceID] =
        closedLookup[cat.ServiceID] === false;
    }
  });
...
async function showQrBadgeModal(
  services,
  isInterpreterNeeded,
  isFastTracked = false,
) {
  ...
  function scaledName(name, maxSize, minSize) {
    ...
    if (len <= 10) return { text: name, size: maxSize * 0.7 };
    if (len <= 12) return { text: name, size: maxSize * 0.58 };
    if (len <= 14) return { text: name, size: maxSize * 0.5 };
  ...
  document.getElementById("qrCardTranslator").style.display =
    isInterpreterNeeded ? "block" : "none";
...
const PASSWORD_SEARCH_PLACEHOLDER =
  '<tr><td colspan="4" class="text-center text-muted p-3">Type a name or email to search clients.</td></tr>';
...
function renderPasswordUsers(users) {
  if (!Array.isArray(users) || users.length === 0) {
    passwordUserTableBody.innerHTML =
      '<tr><td colspan="4" class="text-center text-muted p-3">No clients found.</td></tr>';
    return;
  }

  // For each user, create a table row with their name, DOB, email, and a select button.
  passwordUserTableBody.innerHTML = users
    .map((user) => {
      const middleInitial = user.MiddleInitial
        ? ` ${escapeHtml(user.MiddleInitial)}.`
        : "";
...
function fetchPasswordUsers(query) {
  passwordUserTableBody.innerHTML =
    '<tr><td colspan="4" class="text-center text-muted p-3">Loading users...</td></tr>';
  ...
  fetch("../api/registration-dashboard.php", {
    ...
    .then((data) => {
      if (!data.success) {
        passwordUserTableBody.innerHTML =
          '<tr><td colspan="4" class="text-center text-muted p-3">No users found.</td></tr>';
        Swal.fire({
          icon: "error",
          title: "Search Failed",
...
if (toggleNewUserPasswordBtn && newUserPassword) {
  toggleNewUserPasswordBtn.addEventListener("click", () => {
    ...
    toggleNewUserPasswordBtn.setAttribute(
      "title",
...
if (toggleConfirmUserPasswordBtn && confirmUserPassword) {
  toggleConfirmUserPasswordBtn.addEventListener("click", () => {
    ...
    toggleConfirmUserPasswordBtn.setAttribute(
      "title",
      isPassword ? "Hide password" : "Show password",
    );
    toggleConfirmUserPasswordBtn.setAttribute(
...
function fetchRegistrationQueue() {
  tableBody.innerHTML =
    '<tr><td colspan="3" class="text-center p-3 text-muted">Loading...</td></tr>';

  const status = currentTab === "checked-in" ? "CheckedIn" : "Registered";

  return fetch(
    `../api/registration-dashboard.php?RegistrationStatus=${status}`,
    {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    },
    ...
    .then((data) => {
      if (data.success) {
        ...
        tableBody.innerHTML =
          '<tr><td colspan="3" class="text-center p-3 text-muted">No patients found.</td></tr>';
      }
      ...
      if (
        currentTab !== "checked-in" &&
        statCompCount &&
        data.clientsProcessed !== undefined
      ) {
    ...
    .catch((error) => {
      ...
      tableBody.innerHTML =
        '<tr><td colspan="3" class="text-center p-3 text-danger">Connection error. Please refresh.</td></tr>';
    });
}
...
function populateRegistrationTable(patientsData) {
  ...
  if (patientsData.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="3" class="text-center p-3 text-muted">No patients currently in queue.</td></tr>';
    return;
  }
...
function populateCheckedInTable(patientsData) {
  ...
  if (patientsData.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="3" class="text-center p-3 text-muted">No checked-in patients yet.</td></tr>';
    return;
  }
  ...
  patientsData.sort((a, b) => {
    const lastCmp = (a.LastName || "").localeCompare(
      b.LastName || "",
      undefined,
      { sensitivity: "base" },
    );
  ...
  patientsData.forEach((patient) => {
    ...
    const inProgressServicesEncoded = encodeURIComponent(
      JSON.stringify(patient.inProgressServices || []),
    );
    ...
    const reprintBtnClass = isAbandoned
      ? "btn btn-sm btn-outline-secondary btn-reprint-qr"
      : "btn btn-sm btn-outline-primary btn-reprint-qr";
    const reprintTitle = isAbandoned
      ? "Abandoned clients cannot be reprinted or checked in again."
      : "Reprint QR Badge";
    const rowClass = isAbandoned
      ? "align-middle registration-row-abandoned"
      : "align-middle";
...
function handleReprintQR(e) {
  ...
  try {
    inProgressServices = JSON.parse(
      decodeURIComponent(row.dataset.inProgressServices || "[]"),
    );
  } catch (_) {
    inProgressServices = [];
  }
  ...
  const categoryDescriptions = {
    medical:
      "Choose Exam if this is the <strong>client's first time</strong>, Follow Up if <strong>they've been here before</strong>.",
    dental:
      "Extraction is surgical pulling of teeth, Hygiene is everything else.",
  };

  // Parse existing operational service IDs so we can pre-select radio buttons
  let existingServices = [];
  try {
    existingServices = JSON.parse(row.dataset.services || "[]");
  } catch (_) {}
  ...
  serviceCategories.forEach((cat) => {
    ...
    if (state === 1 && isAvailable && cat.children && cat.children.length > 0) {
      ...
      let radiosHTML = cat.children
        .map((child) => {
          let shortLabel = child.ServiceName;
          if (
            shortLabel.toLowerCase().startsWith(cat.ServiceName.toLowerCase())
          ) {
...
tableBody.addEventListener("click", function (event) {
  ...
  if (serviceBtn) {
    ...
    if (
      serviceBtn.hasAttribute("disabled") ||
      serviceBtn.classList.contains("locked-btn")
    )
      return;
  ...
  if (checkInBtn) {
    ...
    currentClientName =
      currentRowToUpdate.querySelector(".fw-bold.text-dark").innerText;
    ...
    const categoryDescriptions = {
      medical:
        "Choose <strong>Exam</strong> if this is the client's first time, <strong>Follow Up</strong> if they've been here before.",
      dental:
        "<strong>Extraction</strong> is surgical pulling of teeth, <strong>Hygiene</strong> is everything else.",
    };

    serviceCategories.forEach((cat) => {
      const btn = currentRowToUpdate.querySelector(
...
document
  .getElementById("finalizeCheckInBtn")
  .addEventListener("click", async function () {
    ...
    for (const cat of serviceCategories) {
      const catBtn = currentRowToUpdate.querySelector(
        `[title="${cat.ServiceName}"]`,
      );
      if (!catBtn) continue;
    ...
    const hasServiceChanges = !areServiceSelectionsEqual(
      selectedNormalized,
      originalNormalized,
    );
    const hasTranslatorChange =
      isInterpreterNeeded !== originalTranslatorNeeded;
    ...
    const dentalCat = serviceCategories.find((c) =>
      c.ServiceName.toLowerCase().includes("dental"),
    );
    const medicalCat = serviceCategories.find((c) =>
      c.ServiceName.toLowerCase().includes("medical"),
    );
    if (
      dentalCat &&
      medicalCat &&
      categoryStates[dentalCat.ServiceID] &&
      !categoryStates[medicalCat.ServiceID]
    ) {
      ...
      if (result.isDenied) {
        ...
        const medBtn = currentRowToUpdate.querySelector(
          `[title="${medicalCat.ServiceName}"]`,
        );
        ...
        const subSvcContainer = document.getElementById(
          "modalSubServiceSections",
        );
        const alreadyShown = subSvcContainer.querySelector(
          `[data-category="${medicalCat.ServiceID}"]`,
        );
        if (
          !alreadyShown &&
          medicalCat.children &&
          medicalCat.children.length > 0
        ) {
          const radioName = `${medicalCat.ServiceID}Choice`;
          let radiosHTML = medicalCat.children
            .map((child) => {
              let shortLabel = child.ServiceName;
              if (
                shortLabel
                  .toLowerCase()
                  .startsWith(medicalCat.ServiceName.toLowerCase())
              ) {
                shortLabel = shortLabel
                  .substring(medicalCat.ServiceName.length)
                  .replace(/^[\s\-–—]+/, "");
              }
              return `
                    <div class="form-check">
          ...
          newSection.className =
            "mb-4 p-3 border rounded bg-light sub-service-section";
    ...
    const opticalCat = serviceCategories.find((c) =>
      c.ServiceName.toLowerCase().includes("optical"),
    );
    if (
      dentalCat &&
      opticalCat &&
      categoryStates[dentalCat.ServiceID] &&
      categoryStates[opticalCat.ServiceID]
    ) {
      const result = await Swal.fire({
        icon: "error",
      ...
      if (result.isDenied) {
        ...
        const optBtn = currentRowToUpdate.querySelector(
          `[title="${opticalCat.ServiceName}"]`,
        );
        if (optBtn) {
        ...
        const opticalSection = subSvcContainer.querySelector(
          `[data-category="${opticalCat.ServiceID}"]`,
        );
    ...
    fetch("../api/CheckIn.php", {
      ...
      .then(async (data) => {
        if (data.success) {
          ...
          const blockedByInProgress =
            /currently being served|cannot be checked in again/i.test(
              data.message || "",
            );
          Swal.fire({
            icon: blockedByInProgress ? "warning" : "error",
            title: blockedByInProgress
              ? "Client Is Currently In Service"
              : "Check-In Failed",
...
// Sweet Alert Popups
document
  .getElementById("printVolunteerBadgeBtn")
  .addEventListener("click", async function () {
...
(async () => {
  ...
  if (refreshQueueBtn) {
    refreshQueueBtn.addEventListener("click", () =>
      spinRefreshBtn(refreshQueueBtn, fetchRegistrationQueue()),
    );
  }
})();
