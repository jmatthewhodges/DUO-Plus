...
function renderNowServingSeatStats(services) {
  ...
  if (!visibleServices.length) {
    nowServingSeatStatsEl.innerHTML =
      '<p class="mb-0 text-muted small">Seat capacity unavailable.</p>';
    return;
  }
  ...
  visibleServices.forEach((service) => {
    ...
    const parentId = service.ParentServiceID
      ? String(service.ParentServiceID)
      : "";
    ...
    const groupKey =
      normalizedParentId || normalizedServiceId || fallbackGroupName;
    const groupLabel =
      parentName ||
      fallbackGroupName ||
      String(service.ServiceName || "Service");
    const groupPriority =
      servicePriorityMap[parentId] ?? servicePriorityMap[serviceId] ?? 999;
  ...
  const groupMarkup = orderedGroups
    .map((group) => {
      ...
      const serviceLines = orderedServices
        .map((service) => {
          const current = Number(service.SeatsInProgress || 0);
          const max = Number(service.MaxSeats || 0);
  ...
  const renderedGroups =
    groupMarkup ||
    '<div class="now-serving-seat-empty">No active seats to display.</div>';
...
function getFilterIconTag(filterKey, categories) {
  const allowedServiceIDs = (QUEUE_FILTER_SERVICE_IDS[filterKey] || []).map(
    (id) => String(id).toLowerCase(),
  );
  if (!allowedServiceIDs.length) return "";
  const matched = (categories || []).find((cat) =>
    allowedServiceIDs.includes(String(cat.ServiceID || "").toLowerCase()),
  );
...
function matchesSearch(patient, term) {
  if (!term) return true;
  const name =
    `${patient.FirstName} ${patient.MiddleInitial || ""} ${patient.LastName}`.toLowerCase();
  const id = String(patient.ClientID || "").toLowerCase();
  return name.includes(term) || id.includes(term);
}

function applyTableFiltersAndRender() {
  const filtered = (waitListData || []).filter(
    (patient) =>
      matchesQueueFilter(patient, currentQueueFilter) &&
      matchesSearch(patient, currentSearchTerm),
  );
...
function getServiceStatusLabel(status) {
  switch (status) {
    case "Pending":
      return { text: "Pending", class: "bg-light text-dark" };
    case "In-Progress":
      return { text: "In Progress", class: "bg-info text-white" };
    case "Complete":
      return { text: "Complete", class: "bg-success text-white" };
    case "Standby":
      return { text: "Standby", class: "bg-standby" };
    default:
      return { text: "Not Added", class: "bg-light text-muted" };
  }
}
...
function updateAutoRefreshToggleUI() {
  ...
  autoRefreshToggleBtn.setAttribute(
    "aria-pressed",
    autoRefreshEnabled ? "true" : "false",
  );
  autoRefreshToggleBtn.setAttribute(
    "title",
    autoRefreshEnabled ? "Turn off auto refresh" : "Turn on auto refresh",
  );
  autoRefreshToggleBtn.classList.toggle(
    "btn-outline-light",
    autoRefreshEnabled,
  );
  ...
  if (autoRefreshToggleText) {
    autoRefreshToggleText.textContent = autoRefreshEnabled
      ? "Auto: On"
      : "Auto: Off";
  }
}
  ...
  try {
    ...
    if (data.success) {
      Swal.fire({
        icon: "success",
        title: "Skipped",
        text: "Client moved back one position.",
        timer: 1200,
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: data.error || "Could not skip client.",
      });
    }
  } catch (error) {
...
async function abandonClient(clientId) {
  ...
  try {
    ...
    if (data.success) {
      ...
      Swal.fire({
        icon: "success",
        title: "Client Abandoned",
        text: "This client has been permanently removed from the queue.",
        timer: 1500,
        showConfirmButton: false,
      });
      fetchQueueData();
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: data.error || "Could not abandon client.",
      });
    }
  } catch (error) {
  ...
  try {
    ...
    if (data.success) {
      ...
      Swal.fire({
        icon: "error",
        title: "Data Error",
        text: "Could not load waiting room data.",
      });
    }
  } catch (error) {
...
function populateWaitListTable(patients) {
  ...
  if (!patients || patients.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="2" class="text-center p-3 text-muted">No patients in the waiting room.</td></tr>';
  ...
  patients.forEach((patient) => {
    ...
    const inProgressService = (patient.VisitServices || []).find(
      (v) => v.ServiceStatus === "In-Progress",
    );
    const inProgressIconTag = inProgressService
      ? inProgressService.IconTag || ""
      : "";
    const wasSkipped = patient.WasSkipped;
    const orderedVisitServices = [...(patient.VisitServices || [])].sort(
      (a, b) => {
        const pa = servicePriorityMap[a.ServiceID] ?? 999;
        const pb = servicePriorityMap[b.ServiceID] ?? 999;
        if (pa !== pb) return pa - pb;
    ...
    const hasInProgressService = orderedVisitServices.some(
      (vs) => vs.ServiceStatus === "In-Progress",
    );
    const hasFastTrackedService = orderedVisitServices.some((vs) => {
      const isFastTracked = Number(vs.IsFastTracked || 0) === 1;
      const isActive = ["Pending", "Standby", "In-Progress"].includes(
        vs.ServiceStatus,
      );
    ...
    const fastTrackNameBadge = hasFastTrackedService
      ...
      : "";
    const serviceListItems = orderedVisitServices
      .map((vs) => {
    ...
    const finalAvatarIconHTML = isNowServing
      ...
      : wasSkipped
        ...
        : atService
          ...
          : `<i class="bi ${avatarIcon}"></i>`;
    const currentServiceLabel = String(
      inProgressService?.ServiceName || atService || "",
    ).trim();
    const hasStandbyService = orderedVisitServices.some(
      (vs) => vs.ServiceStatus === "Standby",
    );
    const avatarTooltip = (() => {
      if (isNowServing) {
        return currentServiceLabel
          ? `Now serving: Currently at ${currentServiceLabel}`
          : "Now serving client";
      }
    ...
    const nowServingNameBadge = isNowServing
      ? '<span class="waitlist-now-serving-badge">Now serving</span>'
      : "";
    ...
    const btnClass =
      allDone || isAbandoned ? "btn-outline-secondary" : "btn-primary";
    const btnText = allDone ? "View" : isAbandoned ? "View" : "Update";
    const rowClass = [
      "border-bottom",
      rowStatusClass,
      isNowServing ? "waitlist-now-serving-row" : "",
    ]
      .filter(Boolean)
      .join(" ");
    const rowHTML = `
...
tableBody.addEventListener("click", (event) => {
  ...
  if (updateBtn) {
    ...
    const middleInitialDisplay = middleInitialRaw
      ...
      : "";
    document.getElementById("modalPatientName").innerText =
      `${patient.FirstName}${middleInitialDisplay} ${patient.LastName}`;
...
function renderServiceToggles(patient) {
  ...
  if (visitServices.length === 0) {
    container.innerHTML =
      '<p class="text-muted small mb-0">No services assigned to this patient.</p>';
    return;
  }
  ...
  if (patient.IsAbandoned) {
    container.innerHTML =
      '<div class="waitlist-modal-note waitlist-modal-note-danger mb-2">This client is marked as abandoned.</div>';
    visitServices.forEach((vs) => {
      ...
      const dotStatusClass = isInProgress
        ? "is-current"
        : isComplete
          ? "is-complete"
          : "is-pending";
      const rowStatusClass = isInProgress
        ? "is-current"
        : isComplete
          ? "is-complete"
          : isStandby
  ...
  const hasInProgress = visitServices.some(
    (v) => v.ServiceStatus === "In-Progress",
  );
  ...
  visitServices.forEach((vs) => {
    ...
    if (isPending) {
      if (hasInProgress) {
        actionBtn =
          '<button class="btn btn-outline-secondary btn-sm rounded-2 px-3 waitlist-modal-action-btn" disabled title="Check out current service first">Check In</button>';
      } else {
...
async function handleServiceToggle(e) {
  ...
  try {
    ...
    if (data.success) {
      ...
      const updatedPatient = waitListData.find(
        (p) => p.ClientID == currentClientId,
      );
      if (updatedPatient) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: data.error || "Failed to update service.",
      });
      btn.disabled = false;
