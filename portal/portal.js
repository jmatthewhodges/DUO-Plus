 ...
 * ============================================================
 *  File:        portal.js
 *  Description: API Portal behavior for method catalog, testing, and responses.
 *
 *  Last Modified By:  Matthew
 *  Last Modified On:  April 20 @ 5:44 PM
...
const API_METHODS = [
  // ─── Authentication ───────────────────────────────────────────────────
  {
    ...
    description:
      "Authenticate a client with email and password. Returns full client data on success.",
    params: [
      {
        name: "email",
        type: "email",
  ...
  },
  {
    ...
    description:
      "Verify a 6-digit volunteer PIN code. Rate-limited (5 attempts per 15 min).",
    params: [
      {
        name: "pin",
        type: "text",
      ...
      },
      {
        ...
        required: false,
        default: "",
        description: "Page the PIN was entered on (for logging)",
      },
    ],
    testData: {
      pin: "123456",
  ...
  },
  {
    ...
    description:
      "Create a brand-new client with personal info, address, emergency contact, and services. EventID is resolved from the current active event on the server.",
    params: [
      {
        name: "firstName",
        type: "text",
      ...
      },
      {
        ...
        required: false,
        default: "",
        description: "Middle initial (single letter)",
      },
      {
        name: "lastName",
        type: "text",
      ...
      },
      {
        ...
        required: true,
        default: "",
        description: "Login password (will be hashed)",
      },
      {
        name: "noAddress",
        type: "checkbox",
      ...
      },
      {
        ...
        required: false,
        default: true,
        description: "No emergency contact?",
      },
      {
      ...
      },
      {
        ...
        default: '["medical"]',
        description:
          "JSON array of services: medical, optical, dental, haircut",
      },
    ],
    testData: () => {
  ...
  // ─── Event ────────────────────────────────────────────────────────────
  {
    ...
    params: [
      ...
      },
      {
        ...
        required: true,
        default: "",
        description: "Name for the location of the event",
      },
      {
        name: "IsActive",
        type: "checkbox",
        required: true,
        default: "",
        description: "Active status for the event",
      },
    ],
    testData: {
    ...
    params: [
      {
        ...
        required: true,
        default: "",
        description: "ID for the event that is getting the service",
      },
      {
        name: "ServiceID",
        type: "text",
        required: true,
        default: "",
        description: "ID for the service that is getting added to the event",
      },
      {
        name: "MaxCapacity",
        type: "number",
        required: true,
        default: "",
        description: "Max capacity for the service",
      },
      {
        ...
        required: true,
        default: "",
        description: "Number of clients currently assigned to this service",
      },
      {
        name: "IsClosed",
        type: "checkbox",
        ...
        default: "",
        description:
          "Status for if the event is currently offering this service",
      },
    ],
  },
  ...
  // ─── Service ──────────────────────────────────────────────────────────
  {
    ...
    params: [
      ...
      },
      {
        ...
        required: true,
        default: "",
        description: "Offical icon tag for the service",
      },
    ],
    testData: {
  ...
  // ─── Registration Dashboard ───────────────────────────────────────────
  {
    ...
    description:
      'Fetch all clients currently in the "registration" queue with their services and info. No parameters required.',
    params: [],
    testData: {},
  },
  {
    ...
    description:
      "Check a registered client into the waiting room and set their selected services.",
    params: [
      {
        name: "clientID",
        type: "text",
      ...
      },
      {
        ...
        required: false,
        default: '["medical"]',
        description: "Has to match valid ServiceID",
      },
      {
        ...
        required: false,
        default: false,
        description: "Does the client need an interpreter?",
      },
    ],
    testData: {
  ...
  // ─── Analytics ────────────────────────────────────────────────────────
  {
    ...
    description:
      "Create a new statistic for the active event. StatID is auto-generated as a UUID-like ID.",
    params: [
      {
        name: "StatID",
        type: "text",
        ...
        default: "",
        description:
          "Optional override. Leave blank to auto-generate UUID-like ID.",
      },
      {
        name: "StatKey",
        type: "text",
      ...
      },
      {
        ...
        required: true,
        default: "",
        description: "Starting value of the statistic",
      },
    ],
    testData: () => ({
  ...
  },
  {
    ...
    description:
      "Delete only test clients and related rows where tblClients.DateCreated is strictly AFTER the cutoff date.",
    params: [
      {
        name: "cutoffDate",
        type: "date",
        ...
        default: "2026-03-14",
        description:
          "Clients with DateCreated after this date are deleted (strictly AFTER this date).",
      },
    ],
    testData: {
  ...
  },
  {
    ...
    description:
      "Reset analytics stats and availability counts to 0 for one selected event.",
    params: [
      {
        name: "EventID",
        type: "select",
        required: true,
        default: "",
        description: "Select the event to reset.",
        options: [],
      },
    ],
    testData: {
...
(function () {
  ...
  function renderSidebar(filter = "") {
    ...
    API_METHODS.forEach((m) => {
      if (
        ...
        !m.category.toLowerCase().includes(lowerFilter)
      )
        return;
    ...
    if (!html) {
      html =
        '<div style="padding: 2rem; text-align: center; color: #94a3b8; font-size: 0.85rem;">No methods match your search.</div>';
    }
  ...
  function renderMethodDetail(method) {
    ...
    if (method.params.length === 0) {
      paramsHTML =
        '<p class="no-params-msg">This endpoint requires no parameters.</p>';
    } else {
  ...
  async function hydrateResetEventDropdown() {
    ...
    try {
      ...
      const options = data.events.map((ev) => {
        const label =
          ev.label ||
          `${ev.EventDate || "No Date"} - ${ev.LocationName || ev.EventID}`;
  ...
  async function sendRequest(method) {
    ...
    if (
      method.id === "CreateStat" &&
      (!body.StatID || String(body.StatID).trim() === "")
    ) {
  ...
  async function sendBulkRegister(method, count) {
    ...
    lastResponse = {
      status: successCount === count ? 200 : 207,
      data: summary,
      elapsed,
    };
  ...
  function renderResponse(status, data, elapsed) {
    ...
    const statusClass =
      status < 300 ? "success" : status < 500 ? "client-error" : "server-error";
    const statusLabel =
      status < 300 ? "Success" : status < 500 ? "Client Error" : "Server Error";
  ...
  function getTestData(method) {
    return typeof method.testData === "function"
      ? method.testData()
      : method.testData;
  }
